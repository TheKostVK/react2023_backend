    ...
    return (
        <div style={{margin: 20, textAlign: 'center'}}>
            {isLoading && (
                <>
                    <div>
                        <Title level={1}>{<Skeleton.Input  active/>}</Title>
                        <Divider style={{fontSize: 12}}>{<Skeleton.Input active/>}</Divider>
                    </div>
                    <div>
                        <Skeleton />
                    </div>
            ...
            {!isLoading && post && post?.title && post?.short_desc && post?.full_desc &&
                <>
                    <div>
                        ...
                        <Divider style={{fontSize: 12}}>{moment(post.create_date).format('DD.MM.YY HH:m:ss')}</Divider>
                    </div>
                    <div>
